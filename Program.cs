...
string[] smallArray(string[] inArray)
{
    int countSmallString = 0;
    foreach (string item in inArray)
    {
        countSmallString += (item.Length <= 3) ? 1 : 0;
    }
    string[] newArray = new string[countSmallString];
    int i = 0;
    foreach (string item in inArray)
    {
        if (item.Length <= 3)
...
{
    WriteLine($"[{String.Join(", ", item )}] ->[{String.Join(", ", smallArray(item))}]"); //
}
